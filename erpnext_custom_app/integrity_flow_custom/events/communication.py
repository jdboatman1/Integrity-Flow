...
def log_3cx_call(doc, method):
    ...
    """
    if doc.communication_type != "Phone":
        return
    ...
    # Log metadata
    if hasattr(doc, 'duration') and hasattr(doc, 'communication_date'):
        doc.add_comment("Comment", f"Call metadata captured: {doc.duration} seconds on {doc.communication_date}")
